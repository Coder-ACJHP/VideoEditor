//
// TextOverlayRenderingService
// VideoEditor
//  Created by Coder ACJHP on 27.03.2026.

//  `TextOverlayDescriptor` + `TransformEffect` → UIKit preview `UIImage` or export `CALayer` tree.
//  Font size is defined for a 1080p-tall reference and scales with the target height.
...
final class TextOverlayRenderingService: OverlayGenerating, @unchecked Sendable {
    ...
    /// Reference composition height; `descriptor.fontSize` is in points at this height.
    private static let referenceCompositionHeight: CGFloat = 1080

    /// Canvas heights at or below this are treated as timeline strips: fill the cell instead of a tiny normalized box.
    ...
    private func renderPreviewImage(
        ...
        return renderer.image { ctx in
            ...
            paragraph.alignment = descriptor.alignmentMode.nsTextAlignment
    ...
    /// ~36pt-tall timeline strip: with `.identity` the normalized box would be nearly empty, so we paint across the whole cell.
    ...
    private func buildExportLayerTree(
        ...
        textLayer.alignmentMode = descriptor.alignmentMode.caTextLayerAlignment
