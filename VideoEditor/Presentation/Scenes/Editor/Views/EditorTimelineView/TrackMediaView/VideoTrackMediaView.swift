...
//  Requests thumbnails via ThumbnailGenerating and tiles them horizontally.
//  Tile density follows `TimelineConfiguration.thumbnailsPerSecond` (via `mediaStripTileWidth`).
...
final class VideoTrackMediaView: TrackMediaView {
    ...
    private var tileWidth: CGFloat { mediaStripTileWidth }

    private var neededTileCount: Int { mediaStripTileCount }
    ...
    private var thumbnailLoadKey: String {
        let h = contentView.bounds.height
        return "\(contentView.bounds.width)_\(tileImageViews.count)_\(sourceRange.startSeconds)_\(sourceRange.durationSeconds)_\(layout.pointsPerSecond)_\(config.thumbnailsPerSecond)_\(h)"
    }
